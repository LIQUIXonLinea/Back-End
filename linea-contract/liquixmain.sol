...
contract LiquixMarketMaker {
    ...
    function liquidatePosition(
        ...
        require(usdcToBurn > 0, "LnLiquidation: zero amount");

        _liquidatePosition(
            LiquidatePositionParams({user: user, liquidator: msg.sender, usdcToBurn: usdcToBurn}),
            rewardEntryIds
        );
    }

    function liquidatePositionMax(address user, uint256[] calldata rewardEntryIds) external {
        _liquidatePosition(LiquidatePositionParams({user: user, liquidator: msg.sender, usdcToBurn: 0}), rewardEntryIds);
    }
    ...
    function _liquidatePosition(LiquidatePositionParams memory params, uint256[] calldata rewardEntryIds) private {
        ...
        if (params.usdcToBurn == 0) {
            // Liquidate max
            params.usdcToBurn = maxusdcToBurn;
        } else {
            // User specified amount to liquidate
            require(params.usdcToBurn <= maxusdcToBurn, "LnLiquidation: burn amount too large");
        }

        // Burn usdc and update debt
        lnBuildBurnSystem.burnForLiquidation(params.user, params.liquidator, params.usdcToBurn);
        ...
        if (params.usdcToBurn == maxusdcToBurn) {
    ...
    function calculateRewards(
        ...
        uint256 collateralWithdrawalAmount = usdcToBurn.divideDecimal(collateralPrice);
